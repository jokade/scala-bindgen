...
#define DEFINE_ENUM(tpe, id)                                                   \
    enum tpe bindgen_clang_##id() { return id; };
ENUM_INFO(DEFINE_ENUM)

#define PRIMITIVE_GETTER(tpe, name, argType, arg)                              \
    tpe bindgen_clang_##name(argType arg) { return clang_##name(arg); }

#define COPY_GETTER(tpe, name, argType, arg)                                   \
    tpe *bindgen_clang_##name(argType arg) {                                   \
...
#define STRING_GETTER(name, argType, arg)                                      \
    const char *bindgen_clang_##name(argType arg) {                            \
...
CXCursor *bindgen_clang_Cursor_getArgument(CXCursor *cursor, int i) {
...
typedef enum CXChildVisitResult (*bindgen_visitor)(CXCursor *cursor,
                                                   CXCursor *parent,
                                                   void *data);
...
unsigned bindgen_clang_visitChildren(CXCursor *parent, bindgen_visitor visitor,
                                     void *data) {
